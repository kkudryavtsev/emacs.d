...
;; Copyright (C) 1994-1999 Peter S. Galbraith
 
;; Author:    Peter S. Galbraith <GalbraithP@dfo-mpo.gc.ca>
;;                               <psg@debian.org>
;; Created:   16 Sep 1994
;; Version:   3.14 (29 December 1999)
;; Keywords:  find-file, ffap, paths, search

;; RCS $Id: ff-paths.el,v 1.18 1999/12/29 21:00:23 rhogee Exp $
...
;;   ff-paths-display-non-existant-filename
;;   ff-paths-display-non-existant-filename
...
;;   - Added special face to completion buffer for non-existant filename.
...
(defvar ff-paths-non-existant-file-face 'ff-paths-non-existant-file-face 
  "Face to use for message marked for deletion in mh-e folder-mode")
(make-face 'ff-paths-non-existant-file-face)
(if ff-paths-is-XEmacs
    (make-face-bold 'ff-paths-non-existant-file-face nil)
  (make-face-bold 'ff-paths-non-existant-file-face nil t))
(set-face-foreground 'ff-paths-non-existant-file-face "NavyBlue" nil)
...
(defun find-file-using-paths-hook ()
  ...
  (if (or (ff-paths-file-exists-but-cannot-be-read buffer-file-name)
    ...
    (let* ((the-buffer (current-buffer))
      ...
      (if (null matches)
          nil                             ;Return nil
        (if (not ff-paths-display-non-existant-filename)
          ...
          (setq ff-paths-non-existant-filename
            ...
            (let ((minibuffer-setup-hook (cons 'minibuffer-completion-help
                  ...
                  (completion-setup-hook 
                   (append (symbol-value 'completion-setup-hook)
                           (list 'ff-paths-fontify-non-existant-filename))))
              (setq the-name
                    (or (and (string-equal "18" (substring emacs-version 0 2))
...
(defun ff-paths-fontify-non-existant-filename ()
  "Fontify the non-existing filename in *Completions* if using window-system"
  (cond 
   ((and window-system
         ff-paths-display-non-existant-filename
         (boundp 'ff-paths-non-existant-filename)
         ff-paths-non-existant-filename)
    (save-excursion
      (set-buffer standard-output)
      (goto-char (point-min))
      (if (search-forward ff-paths-non-existant-filename nil t)
          (progn
            (put-text-property (match-beginning 0) (match-end 0) 
                               'face 'ff-paths-non-existant-file-face)
            (goto-char (point-min))
...
;;    Version: $Id: ff-paths.el,v 1.18 1999/12/29 21:00:23 rhogee Exp $
;;    Keywords: wp
